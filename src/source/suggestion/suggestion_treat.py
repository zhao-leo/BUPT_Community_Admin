...
from API import suggestiontreat

from source.layout.page_layout import PageLayout

class SuggestionReplyPage(PageLayout):
    def __init__(self):
        super().__init__('建议列表-待回访-{}')
        self.res=get_suggestion(suggestiontreat())
    ...
    def content(self):
        if self.res.get('data'):
            with ui.column().style('width:100%'):
                ...
                rows = [{'id':i.get('id'),'name':i.get('sugg_name'),'content':i.get('sugg_title'),'site':i.get('sugg_site'),'time':i.get('sugg_sub_time').split('T')[0],'link':'/suggestion/treated/{}'.format(i.get('id'))} for i in self.res.get('data')]
