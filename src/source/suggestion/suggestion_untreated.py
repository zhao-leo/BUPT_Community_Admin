    ...
    def content(self):
        if self.res.get('data'):
            with ui.column().style('width:100%'):
                ...
                rows = [{'id':i.get('id'),'name':i.get('sugg_name'),'content':i.get('sugg_title'),'site':i.get('sugg_site'),'time':i.get('sugg_sub_time').split('T')[0],'link':'/suggestion/untreated/{}'.format(i.get('id'))} for i in self.res.get('data')]
