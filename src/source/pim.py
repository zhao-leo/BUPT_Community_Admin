    ...
    def content(self):
        with ui.card().style("width:100%;height:100%"):
            ...
            with ui.row().style('width:100%'):
                ...
                #     with ui.dialog().style('width:50%') as dialog:
                #         ui.date().props("range")
                #     ui.button("下载Excel表格").on('click',lambda:dialog.open())
                    ...
                    #         datemin = '2020/01/01'
                    #         if date1.value!='':
                    #             datemin = date1.value.replace('-','/')
                    #         with ui.date().bind_value(date2).props(''':options="date2 => date2 >= '{}'"'''.format(datemin)):
                    #             with ui.row().classes('justify-end'):
                    #                 ui.button('关闭', on_click=menu.close).props('flat')
