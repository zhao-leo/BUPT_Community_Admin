...
class ComplaintReplyPage(PageLayout):
    def __init__(self):
        ...
        self.pagenum = 1
        self.res=get_complaint(complaintHistory(),{'pagenum':self.pagenum,'pagesize':8})
    ...
    def content(self):
        with ui.card().style('width:100%;height:auto'):
            with ui.row().style('width:100%;height:auto'):
                ...
                ui.label('投诉人').style('width:12%;height:auto')
                    ...
                    for i in self.res.get('data'):
                        with ui.card().style('width:100%;height:auto'):
                                with ui.row().style('width:100%;height:auto'):
                                    ...
                                    ui.button("点击前往",on_click=lambda i=i :ui.navigate.to('/complaint/finished/{}'.format(i.get('id')),new_tab=True)).style('width:8%;')
            else:
